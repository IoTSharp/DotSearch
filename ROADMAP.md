...
**功能清单**：
- 倒排表 `(field, term) -> postings`
- BM25 评分（k1=1.2、b=0.75 默认）
- `TermQuery` / `AndQuery` / `OrQuery` 三种节点
- `UnicodeTokenizer`：按 Unicode 类别切分 + 小写化
- `CjkBigramTokenizer`：零词典 CJK 二元切分
...
**功能清单**：
- 不可变段（segment）+ 后台合并
- doclist 采用 VarInt + delta encoding 编码
- `manifest.json` 描述所有活动段
- Tombstone 处理删除
- 单目录持久化布局，与 IoTSharp 旗下其他库（如 DotVector）对齐
...
**功能清单**：
- 词典编译工具：把行格式词表编译成 DAT 二进制，作为 embedded resource 嵌入
- 短语查询、`NEAR(a b, k)` 近邻查询
- BM25F 列权重评分
- 同义词 / 拼音 token filter
