...
内存实现使用嵌套字典，持久化实现使用 LSM 风格的多段（segment）+ manifest，
...
* `InMemoryFullTextIndex` 内部使用一把 `System.Threading.Lock` 串行化写入与查询。
* `PersistentFullTextIndex` 以不可变段 + manifest 组织数据，写入、删除、查询由实例锁保护；
  段合并会重写当前可见文档并回收 tombstone。
