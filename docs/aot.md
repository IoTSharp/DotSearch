...
* **词典型分词器**：词典走 embedded resource，启动一次性加载到只读结构，
  运行时不再有反射或动态解析。
* **未来 DAT 词典**：编译期把行格式词表压成 Double-Array Trie 二进制，
  运行时仅做结构化读取，AOT 友好。
...
在构建阶段持续监控，等到 M6 再切到 `PublishAot=true` 做端到端验证。
...
- [ ] `dotnet build -c Release` 全仓零警告
- [ ] `dotnet publish -c Release -p:PublishAot=true` 在 Linux x64 / arm64 通过
- [ ] 启动后做一次端到端 gRPC 调用：`Ping` / `CreateIndex` / `Upsert` / `Search` 均成功
- [ ] 二进制大小、冷启动时间记录到 `benchmarks/` 报告
